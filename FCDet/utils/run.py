...
class Run():
    ...
    def main(self):
        self.model = FCDet(self.dataset,self.config)
        ...
        if self.mode=='train':
            ...
            trainer=Trainer(model=self.model,device=self.device,lr=self.lr,dataloaders=dataloaders,epoches=self.epoches,model_name='FDet',save_predict_result_path=save_predict_result_path,beta_c=self.alpha,beta_n=self.beta,early_stop=self.early_stop,save_param_path=self.path_ckp+self.dataset+"/",writer=SummaryWriter(self.path_tb+self.dataset+"/"))
            ckp_path=trainer.train()
            try:
                result=trainer.test(ckp_path)
                with open(self.config['result_path'], 'a+') as f:
                    f.write(f"alpha: {self.config['alpha']}\tbeta: {self.config['beta']}\t")
                    f.write(f"topk_token: {self.config['topk_token']}\ttopk_patch: {self.config['topk_patch']}\n")
                    f.write(str(result))
                    f.write('\n')
            except Exception:
                with open(self.config['result_path'], 'a+') as f:
